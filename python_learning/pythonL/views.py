...
def basis(request, pk):
    ...
    if question.category % 4 == 0: #コードで入力
        ...
        for i in range(len(d_list)):
            ...
            pre_code = pre_code.replace(f'/radc{i}/', str(d_list[i]))
            pre_visual = pre_visual.replace(f'/radc{i}/', str(d_list[i]))
            q_data = q_data.replace(f'/radc{i}/', str(d_list[i]))
            e_answer = e_answer.replace(f'/radc{i}/', str(d_list[i]))
        ...
        c_output = op.stdout
    elif question.category % 2 == 0: #直接入力
        ...
        c_output = question.c_output.split('/nct/')
        e_answer = question.e_answer.split('\n/nct/')
        c_output, e_answer = c_output[0], e_answer[0]
